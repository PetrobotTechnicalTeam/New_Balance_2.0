...
#define STD_LOOP_TIME1 8
#define STD_LOOP_TIME2 8
int timegoneby = STD_LOOP_TIME1;
int manualtime = STD_LOOP_TIME2;
int preleftrpm, prerightrpm;
...
float kp = 50;//70;// 57;//37.5;//22;//24.8;//45MAX:116
float ki = 0;//0.00001;
float kd =35000;//10800;//6600;//10000;
float lastpitch;      // keeps track of errors over time
float iterm;          // Used to accumulate error (intergral)
float targetangle = 0; // adjusted according to centre of gravity
double thistime = 0;
double lasttime = 0;

// You can Turn off YAW control, by setting
// the Tp and Td constants below to 0.
float Tp = 1;              // Yaw Proportional Tuning Parameter
...
//             MPU PROCESS                //
//========================================//
float MPU6050_Process(float* a, float* b) {
  int error;
  double dT;
  accel_t_gyro_union accel_t_gyro;
  // Read the raw values.
...
int PID(float pitch) {
  ...
  //-------calculate time since lasttime PID called [10ms]-----//
  thistime = millis();
  ...
  //------------conversion form angle error to PWM-------------//
  //----------------------set PWM value------------------------//
  float PIDvalue = pterm + iterm - dterm;

  //-------------Set PWM within motor speed range--------------//
  if (PIDvalue > 255) {
    PIDvalue = 255;
...
void MoveControl(int pidValue,int pitch, float yaw , int* left, int* right){
    ...
    //Recovery from free fall
    //------------------------------------------
    /*
    if (pitch>2) left_PWM = -200;
    else if (pitch<-2) left_PWM = 200;
    if (pitch>2) right_PWM = -200;
    else if (pitch<-2) right_PWM = 255;
    */
    *left = left_PWM;
    *right= right_PWM;
...
//            MOTOR MOVE               //
//========================================//
void move_motors(int motor,int rpm) {
...
//========================================//
//    Time Keeper [100hz = 0.01sec]       //
//========================================//
void timekeeper() {
  timegoneby = millis() - loopstarttime;

  if (timegoneby < STD_LOOP_TIME1) {
    delay(STD_LOOP_TIME1 - timegoneby);
   Serial.print("      OOO     ");
   Serial.print("interval time : ");
   Serial.println (STD_LOOP_TIME1 - timegoneby );
  } else {
    Serial.println("      XXX     ");
  }
...
void loop () {
  ...
  //===============================SET POINT=====================================//
        if(state==0){
            ...
            move_motors(1,0);
            MPU6050_Process(&a,&b);
            if ((-1 <= targetangle) && (targetangle <= 1)){
                  targetangle = a;
                  targetYaw   = b;
  ...
  if (start == 1 && state==1) { 
      MPU6050_Process(&a,&b);
                ...
                //Serial.print("Yaw   : ");
                //Serial.println(b,2);
       if ( (a < targetangle + 50) && (a > targetangle - 50) ) {
         ...
         int motor_output = PID(a);
                //Serial.println(motor_output);
                Serial.print("\t");
                Serial.println(a,2);
          MoveControl(motor_output,a,b,&leftRPM,&rightRPM);
          move_motors(1,leftRPM);
          move_motors(0,rightRPM);
                // Serial.println(">>>> Balancing System Active <<<<");
                // Serial.print("PID : ");
                // Serial.println(motor_output);
        } else {
               //Serial.println(">>>> Balancing System Halt <<<<");
          move_motors(0,0);
          move_motors(1,0);
        }
         timekeeper();
